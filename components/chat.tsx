...
export function Chat({
	...
	const { open: isSidebarOpen, openMobile: isSidebarMobileOpen } =
		useSidebar();
	const isNewChat = initialMessages.length === 0;
	const shouldRefreshHistory =
		(isSidebarOpen || isSidebarMobileOpen) && isNewChat;
	...
	const setMessagesRef = useRef<UseChatHelpers<ChatMessage>["setMessages"]>();
			...
			if (dataPart.type === "data-appendMessage") {
				try {
					const message = JSON.parse(
						(dataPart as unknown as { data: string }).data
					);
