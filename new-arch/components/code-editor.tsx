 ...
 * @module new-arch/components/code-editor
 *
 * A CodeMirror-based code editor with syntax highlighting and dark theme.
...
function PureCodeEditor({
    ...
    useEffect(() => {
        ...
        const {
            ...
            EditorView: EditorViewModule,
            basicSetup,
            python,
            oneDark,
        } = modules;

        const startState = EditorStateModule.create({
            doc: content,
            extensions: [basicSetup, python(), oneDark],
        });

        editorRef.current = new EditorViewModule({
            state: startState,
    ...
    useEffect(() => {
        ...
        const updateListener = EditorViewModule.updateListener.of((update) => {
            if (update.docChanged) {
                const transaction = update.transactions.find(
                    (tr) => !tr.annotation(TransactionModule.remote)
                );
        ...
        const newState = EditorStateModule.create({
    ...
    useEffect(() => {
        ...
        const { Transaction: TransactionModule } = modules;
        ...
        if (status === "streaming" || currentContent !== content) {
            const transaction = editorRef.current.state.update({
                ...
                annotations: [TransactionModule.remote.of(true)],
            });
