...
test.describe
    .serial("Chat Title Persistence", () => {
        test("Chat title should be preserved when sending subsequent messages", async ({
            ...
            const historyResponse = await adaContext.request.get(
                "/api/history?limit=1"
            );
            ...
            while (title === "New Chat" && attempts < 10) {
                ...
                const pollResponse = await adaContext.request.get(
                    "/api/history?limit=1"
                );
            ...
            if (title === "New Chat") {
                console.warn(
                    "Could not test title preservation because title remained 'New Chat'"
                );
                return;
            }
            ...
            const finalHistoryResponse = await adaContext.request.get(
                "/api/history?limit=1"
            );
