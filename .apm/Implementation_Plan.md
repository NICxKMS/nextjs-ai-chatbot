...
**Last Modification:** Plan updated with 12 critical/high fixes from deep analysis.
**Project Overview:** Complete architecture overhaul migrating from archive/oldapp to v6 architecture with Repository Pattern, Feature Modules, and Slim Routes. UI functionality must remain visually identical. ~274 target files across 7 phases, 75 tasks.
...
- All public APIs (exported functions, classes, interfaces) MUST have TSDoc comments
- TSDoc must include: @param, @returns, @throws, @example for complex functions
- Components must document props via interface comments or Storybook

---

## Phase 0: Prerequisites
*Goal: Ensure root configuration files are in place*
*Tasks: 2 | Est. Duration: 2-3 hours*
...
7. Create `.commitlintrc.js` with Conventional Commits rules
8. Create `.husky/commit-msg` pre-commit hook for commitlint
9. Document commit message format in `CONTRIBUTING.md` (feat:, fix:, docs:, chore:)

### Task 0.2 – Database Migration Script - Agent_Infrastructure
**Objective:** Create migration scripts for schema changes between v5 and v6.
**Output:** Migration scripts in `drizzle/migrations/`.
**Guidance:** No dependencies. Run before Phase 1 to prepare database.

1. **Review database schema** in `archive/oldapp/lib/db/schema.ts` for current state
2. Create migration: rename `documents` table to `artifacts` if applicable
3. Create migration: add any new columns required by v6 schema
4. Create migration: add indexes for performance
5. Create rollback migration for each change
6. Test migrations on local database copy
7. Document migration order in `drizzle/README.md`
8. Create `drizzle/seed.ts` with sample data for local development:
   - 2 test users (admin@test.com, user@test.com)
   - 5 sample chats with messages
   - 3 sample artifacts (text, code, image)
9. Add `pnpm db:seed` script to package.json
10. Document seed usage in `drizzle/README.md`

---

## Phase 1: Infrastructure Foundation
*Goal: Establish foundational lib/ modules that all other layers depend on*
*Tasks: 13 | Est. Duration: 11-15 hours*
...
1. **Review corresponding implementation** in `archive/oldapp/lib/errors.ts` for reference patterns and existing error handling approaches
2. Create `AppError` base class with `code`, `message`, `statusCode` properties
...
### Task 1.2b – User-Friendly Error Messages - Agent_Infrastructure
**Objective:** Map technical errors to user-friendly messages with i18n support.
**Output:** `lib/errors/messages.ts`, error message constants.
**Guidance:** **Depends on: Task 1.2 Output**.

1. **Analyze `archive/oldapp/lib/errors.ts`** for existing error patterns and messages
2. Create `lib/errors/messages.ts` with user-facing error messages for each error code
3. Map AppError codes to friendly messages:
...
4. Support i18n-ready structure (messages as keys, not hardcoded strings)
5. Create `getErrorMessage(error: AppError, locale?: string)` utility
6. Add unit tests for error message mapping
...
1. **Review corresponding implementation** in `archive/oldapp/lib/constants.ts` for reference patterns and existing constant values
2. Create `CACHE_TTL` object with entity-specific TTLs (chat: 3600, message: 1800, user: 7200, list: 300)
...
1. **Review corresponding implementation** in `archive/oldapp/lib/utils.ts` for reference patterns and existing utility functions
2. Create `lib/utils/cn.ts` with `cn()` function using clsx + tailwind-merge
...
1. **Study `archive/oldapp/lib/db/`** for existing database patterns, schema definitions, and client configuration
2. Create `lib/db/client.ts` with Drizzle client using connection pooling
...
1. **Review corresponding implementation** in `archive/oldapp/lib/cache/` for reference patterns and existing cache key structures
2. Create `lib/cache/client.ts` with Upstash Redis client from environment variables
...
- [ ] Create warmCache utility for frequently accessed data
- [ ] Implement cache warming on app startup for active user sessions
- [ ] Add cache warming trigger after cache invalidation
- [ ] Document warming patterns in cache module
...
1. **Review corresponding implementation** in `archive/oldapp/lib/auth/` for reference patterns, provider setup, and session configuration
...
1. **Review corresponding implementation** in `archive/oldapp/lib/auth/` for reference patterns, session handling, and guard implementations
2. Create `lib/auth/session.ts` with getSession, requireSession, isAuthenticated, getSessionUser, guest support
...
1. **Review corresponding implementation** in `archive/oldapp/lib/api/` and `archive/oldapp/lib/api-context.ts` for reference patterns and existing validation approaches
2. Create `lib/api/response.ts` with success(), error(), paginated(), stream() response builders
...
1. **Study `archive/oldapp/lib/db/` and `archive/oldapp/lib/data/`** for existing data patterns, query structures, and entity relationships
2. Define `IReadRepository<T>` interface with findById, findMany, exists, count
...
1. **Study `archive/oldapp/lib/db/` and `archive/oldapp/lib/data/`** for existing chat data patterns, queries, and entity handling
2. Extend BaseRepository<Chat, NewChat, UpdateChat>
...
1. **Study `archive/oldapp/lib/db/` and `archive/oldapp/lib/data/`** for existing message data patterns and bulk operation handling
2. Extend BaseRepository<Message, NewMessage, UpdateMessage>
...
1. **Study `archive/oldapp/lib/db/` and `archive/oldapp/lib/data/`** for existing user data patterns and auth-related queries
2. Extend BaseRepository<User, NewUser, UpdateUser>
...
1. **Study `archive/oldapp/lib/db/` and `archive/oldapp/lib/data/`** for existing artifact data patterns and versioning support
2. **Analyze `archive/oldapp/artifacts/`** to understand artifact types and data structures
3. Extend BaseRepository<Artifact, NewArtifact, UpdateArtifact>
...
1. **Study `archive/oldapp/lib/db/` and `archive/oldapp/lib/data/`** for existing vote data patterns and upsert handling
2. Use composite key: chatId + messageId
...
1. **Study `archive/oldapp/lib/db/` and `archive/oldapp/lib/data/`** for existing suggestion data patterns
2. Implement findByDocumentId, createMany, deleteByDocumentId, deleteAfterTimestamp
...
1. **Study `archive/oldapp/lib/db/` and `archive/oldapp/lib/data/`** for existing query patterns, joins, and aggregation approaches
2. Create `chat.queries.ts` with withMessageCount, withLatestMessage, searchByContent, withinDateRange, aggregateByPeriod
...
1. **Study `archive/oldapp/lib/middleware/`** for existing rate limiting patterns and strategies
2. Create `limiter.ts` with checkLimit function using Ratelimit sliding window
...
1. **Study `archive/oldapp/lib/middleware/`** for existing middleware patterns, rate limiting integration, and request handling
2. Create middleware rate limiting utilities that integrate with strategies
...
*Tasks: 17 | Est. Duration: 20-28 hours*
...
1. **Analyze `archive/oldapp/lib/ai/`** to understand existing AI integration patterns, streaming logic, and chat flow
2. Create `stream-chat.action.ts` with streamChat integrating AI SDK streamText
...
1. **Analyze `archive/oldapp/components/chat.tsx`** to understand UI logic, props, state management, and event handlers
2. **Analyze `archive/oldapp/components/messages.tsx`** to understand list rendering, virtualization, and scroll behavior
3. **Analyze `archive/oldapp/components/message.tsx`** to understand role-based styling and markdown rendering
4. **Document key functionality that must be preserved** from each component
5. Create `chat.tsx` main container orchestrating messages, input, streaming state
...
- [ ] All props from original component preserved
- [ ] Event handlers produce same behavior
- [ ] Edge cases (empty state, error state, loading) handled
- [ ] Visual appearance matches original (manual comparison)
...
1. **Analyze `archive/oldapp/components/message-editor.tsx`** to understand inline editing logic, props, and state management
2. **Analyze `archive/oldapp/components/message-actions.tsx`** to understand action handlers and UI patterns
3. **Analyze `archive/oldapp/components/message-reasoning.tsx`** to understand AI reasoning display logic
4. **Document key functionality that must be preserved** from each component
5. Create `message-editor.tsx` for inline editing of user messages
...
1. **Analyze `archive/oldapp/components/chat-header.tsx`** to understand header layout, model selector integration, and event handlers
2. **Analyze `archive/oldapp/components/greeting.tsx`** to understand welcome message logic and user flow
3. **Analyze `archive/oldapp/components/toolbar.tsx`** and `archive/oldapp/components/visibility-selector.tsx` for toolbar patterns
4. **Document key functionality that must be preserved** from each component
5. Create `chat-header.tsx` with model selector integration
...
1. **Analyze `archive/oldapp/hooks/use-messages.tsx`** to understand message state management and scroll behavior
2. **Analyze `archive/oldapp/components/data-stream-handler.tsx`** and `archive/oldapp/components/data-stream-provider.tsx` for SSE stream patterns
3. Create `use-chat.ts` extending AI SDK useChat with custom state management
...
1. **Analyze `archive/oldapp/artifacts/actions.ts`** to understand existing artifact action patterns, versioning logic, and data flow
2. Create `create-artifact.action.ts` for artifact creation
...
1. **Analyze `archive/oldapp/components/artifact.tsx`** to understand panel layout, type-based editor selection, and state management
2. **Analyze `archive/oldapp/components/artifact-actions.tsx`** for toolbar patterns and action handlers
3. **Analyze `archive/oldapp/components/artifact-close-button.tsx`** and `archive/oldapp/components/artifact-error-boundary.tsx`
4. **Document key functionality that must be preserved** from each component
5. Create `artifact-panel.tsx` main container with type-based editor selection
...
1. **Analyze `archive/oldapp/components/text-editor.tsx`** to understand TipTap integration, extensions used, props, state management, and debounce patterns
2. **Document key functionality that must be preserved:** markdown rendering, editing modes, onChange behavior
3. Integrate TipTap with required extensions
...
1. **Analyze `archive/oldapp/components/code-editor.tsx`** to understand CodeMirror integration, language support, theme setup, and event handlers
2. **Analyze `archive/oldapp/components/console.tsx`** to understand console output patterns (ConsoleOutput, ConsoleOutputContent types)
3. **Document key functionality that must be preserved:** syntax highlighting, language detection, line numbers, onChange behavior
4. Integrate CodeMirror with syntax highlighting
...
1. **Analyze `archive/oldapp/components/image-editor.tsx`** to understand image manipulation logic, canvas operations, and UI controls
2. **Analyze `archive/oldapp/components/sheet-editor.tsx`** to understand react-data-grid integration, column definitions, and data handling
3. **Document key functionality that must be preserved** from each editor
4. Create `image-editor.tsx` for image manipulation
...
1. **Analyze `archive/oldapp/artifacts/`** directory to understand handler patterns for each artifact type (text/, code/, image/, sheet/)
2. **Document key functionality that must be preserved:** streaming behavior, state updates, error handling
3. Create base.handler.ts abstract handler class
...
1. **Analyze `archive/oldapp/hooks/use-artifact.ts`** to understand artifact state management, version tracking, and persistence logic
2. **Analyze `archive/oldapp/lib/types.ts`** for ArtifactKind, UIArtifact, and related type definitions
3. Create `use-artifact.ts` managing artifact state, versions, persistence
...
1. **Analyze `archive/oldapp/components/auth-form.tsx`** to understand form logic, validation, error handling, and submit flow
2. **Analyze `archive/oldapp/components/auth-provider.tsx`** to understand session context and authentication state management
3. **Document key functionality that must be preserved:** login/register modes, error display, loading states
4. Create `login.action.ts`, `register.action.ts`, `logout.action.ts`, `reset-password.action.ts` server actions
...
1. **Analyze `archive/oldapp/components/app-sidebar.tsx`** to understand sidebar layout, toggle behavior, and navigation structure
2. **Analyze `archive/oldapp/components/sidebar-history.tsx`** and `archive/oldapp/components/sidebar-history-item.tsx` for history list patterns
3. **Analyze `archive/oldapp/components/sidebar-toggle.tsx`** and `archive/oldapp/components/sidebar-user-nav.tsx` for toggle and user navigation
4. **Document key functionality that must be preserved:** chat history display, search, delete actions, user navigation
5. Create `get-history.action.ts`, `delete-chat.action.ts` server actions
...
1. **Analyze `archive/oldapp/components/settings/`** directory to understand settings UI patterns, form structure, and preference handling
2. **Analyze `archive/oldapp/lib/settings/`** for settings types, defaults, and storage patterns
3. Create `update-settings.action.ts` server action
...
1. **Analyze `archive/oldapp/components/multimodal-input.tsx`** to understand input logic, auto-resize behavior, file attachment handling, and submit flow
2. **Analyze `archive/oldapp/components/preview-attachment.tsx`** for attachment preview patterns
3. **Analyze `archive/oldapp/lib/files.ts`** for file handling utilities, validation, and mime type detection
4. **Document key functionality that must be preserved:** text input, auto-resize, file drag-and-drop, attachment preview
5. Create `MultimodalInput.tsx` with text input, auto-resize, file attachment
...
1. **Analyze `archive/oldapp/lib/ai/tools/`** directory to understand existing tool implementations, schemas, and execution patterns
2. **Analyze `archive/oldapp/components/weather.tsx`** for weather tool UI integration
3. **Document key functionality that must be preserved:** tool schemas, execution flow, result handling
4. Create `weather.tool.ts` for weather fetching
...
**Tool Error Handling:**
- [ ] Add tool execution timeout (30s default, configurable)
- [ ] Implement graceful failure messaging to user
- [ ] Add tool result schema validation before processing
- [ ] Create ToolExecutionError class for typed errors
- [ ] Add retry logic for transient failures (max 2 retries)
...
1. **Analyze `archive/oldapp/components/data-stream-provider.tsx`** to understand SSE connection lifecycle, context structure, and state management
2. **Analyze `archive/oldapp/components/data-stream-handler.tsx`** to understand event consumption, state updates, and error handling
3. **Document key functionality that must be preserved:** connection lifecycle, event types, reconnection logic
4. Create `DataStreamProvider.tsx` managing SSE connection lifecycle and context
...
*Tasks: 13 | Est. Duration: 16-22 hours*

**RSC vs Client Component Matrix:**
| Component | Type | Reason |
|-----------|------|--------|
| ChatContainer | Client | useState, useEffect for streaming |
| Messages | Client | Scroll position, animations |
| Message | Client | Action handlers |
| Sidebar | Client | Toggle state |
| Layout | Server | No interactivity, SEO |
| ErrorBoundary | Client | Error state |
| ThemeProvider | Client | Context required |
...
1. **Study `archive/oldapp/components/elements/`** source files to understand component structure, props interfaces, and dependencies before copying
2. **Document the 31 element files** and their purposes to ensure complete migration
3. Create `components/ai-elements/` directory
...
1. **Study `archive/oldapp/components/elements/`** to understand primitive component APIs and composition patterns
2. **Analyze `archive/oldapp/components/message.tsx`** and `archive/oldapp/components/message-reasoning.tsx`** for chat/reasoning UI patterns
3. Create chat wrappers: AIMessage, AIChatInput, AIConversation
...
1. **Study `archive/oldapp/components/elements/`** for code block, image, and preview primitives
2. **Analyze `archive/oldapp/components/code-editor.tsx`** for code display patterns and actions (copy, run)
3. Create AICodeBlock wrapper with copy toast, run action
...
1. **Study `archive/oldapp/components/elements/`** for workflow and utility primitives (loading, suggestions, etc.)
2. **Analyze `archive/oldapp/components/suggested-actions.tsx`** and `archive/oldapp/components/suggestion.tsx`** for suggestion UI patterns
3. Create workflow wrappers: AIPlan, AITask, AIQueue, AICheckpoint
...
1. **Study `archive/oldapp/components/ui/`** source files to understand existing component configurations, customizations, and theme integration
2. **Document which components have customizations** that must be preserved vs. standard shadcn components
3. Copy/regenerate button, input, textarea, dialog components
...
- Create consistent Skeleton variants (ChatSkeleton, MessageSkeleton, SidebarSkeleton)
- Create LoadingSpinner with size variants (sm, md, lg)
- Create ShimmerEffect for list loading
- Document loading state usage pattern in component index
...
1. **Study `archive/oldapp/components/` source files** for layout components: auth-form.tsx, app-sidebar.tsx, sidebar-toggle.tsx, sidebar-user-nav.tsx, theme-provider.tsx, toast.tsx, icons.tsx, version-footer.tsx
2. **Analyze each component** to understand props, state management, and integration patterns
3. **Document key functionality that must be preserved** from each layout component
4. Create `auth-form.tsx` consolidated login/register form with mode prop
...
### Task 4.4b – Create Document Components - Agent_Components
**Objective:** Migrate document-related components for artifact display and versioning.
**Output:** `components/document/` with document.tsx, diffview.tsx, document-preview.tsx, document-skeleton.tsx.
**Guidance:** **Depends on: Task 4.3 Output**. Migrate from archive/oldapp/components/.

1. **Analyze `archive/oldapp/components/document.tsx`** to understand document rendering logic, state management, and integration points
2. **Analyze `archive/oldapp/components/diffview.tsx`** to understand version diff display, highlighting, and comparison logic
3. **Analyze `archive/oldapp/components/document-preview.tsx`** to understand preview rendering and iframe handling
4. **Analyze `archive/oldapp/components/document-skeleton.tsx`** to understand loading states
5. **Document key functionality that must be preserved** from each component
6. Create `components/document/` directory
7. Create `document.tsx` main document renderer with content display
8. Create `diffview.tsx` version diff view with line-by-line comparison
9. Create `document-preview.tsx` document preview component
10. Create `document-skeleton.tsx` loading skeleton component
11. Create `components/document/index.ts` barrel export
...
1. **Study `archive/oldapp/hooks/`** source files to understand hook implementations, dependencies, and usage patterns
2. **Analyze each hook file:** use-artifact.ts, use-chat-visibility.ts, use-messages.tsx, use-mobile.ts, use-optimistic-chats.tsx, use-scroll-to-bottom.tsx, use-window-size.ts
3. **Document key functionality that must be preserved** from each hook
4. Create `use-debounce.ts` debounced value hook
...
### Task 4.5b – Accessibility (a11y) Compliance - Agent_Components
**Objective:** Add accessibility features across all interactive components.
**Output:** ARIA labels, keyboard navigation, focus management, screen reader support.
**Guidance:** **Depends on: Task 4.3, 4.4 Output**. Follow WCAG 2.1 AA guidelines.

1. **Audit all interactive elements** for ARIA label requirements
2. Add ARIA labels to chat input, message actions, sidebar toggles, artifact panels
3. Implement keyboard navigation for chat history (↑/↓ arrows)
4. Add focus trap to dialogs and modals
5. Implement screen reader announcements for streaming AI responses
6. Add skip-to-content link in layout
7. Verify color contrast ratios meet AA standard (4.5:1)
8. Add role attributes to landmark regions (main, nav, aside)
...
1. **Study `archive/oldapp/lib/settings/`** for existing settings types, defaults, and storage patterns
2. **Analyze `archive/oldapp/lib/motion.tsx`** for Framer Motion animation utilities and presets
3. **Analyze `archive/oldapp/lib/ui/`** (if exists) for UI state management patterns
4. Create `lib/settings/types.ts` with SamplingSettings, SystemPromptSettings, ModelSettings
...
1. **Study `archive/oldapp/lib/ai/`** source files to understand provider configurations, model registry, and prompt templates
2. **Analyze existing AI integration patterns** and tool definitions
3. **Document key functionality that must be preserved:** model configurations, provider setup, system prompts
4. Create `lib/ai/registry.ts` with model registration and lookup
...
### Task 4.7b – Token Management - Agent_Components
**Objective:** Implement token counting and context window management.
**Output:** `lib/ai/tokens.ts`, `lib/ai/context-manager.ts`.
**Guidance:** **Depends on: Task 4.7 Output**. Use tiktoken for accurate counting.

1. **Analyze `archive/oldapp/lib/ai/`** for any existing token handling patterns
2. Create `lib/ai/tokens.ts` with countTokens, estimateTokens functions using tiktoken
3. Define context window limits per model (GPT-4: 128k, GPT-3.5: 16k, Claude: 200k)
4. Create `lib/ai/context-manager.ts` with truncateContext, prioritizeMessages functions
5. Implement smart message truncation (keep system prompt + recent messages)
6. Add token usage tracking integration with quota system
7. Create type definitions for TokenUsage, ContextWindow
...
1. **Analyze `archive/oldapp/app/layout.tsx`** to understand provider hierarchy, font loading, and metadata configuration
2. **Analyze `archive/oldapp/app/globals.css`** for Tailwind directives and custom CSS variables
3. **Analyze `archive/oldapp/app/global-error.tsx`** for error boundary patterns
4. Create `app/layout.tsx` with ThemeProvider, AuthProvider, SettingsProvider
...
1. **Analyze `archive/oldapp/app/(auth)/`** directory to understand auth route structure, layout patterns, and page implementations
2. Create `app/(auth)/layout.tsx` centered card layout
...
1. **Analyze `archive/oldapp/app/(chat)/`** directory to understand chat route structure, sidebar integration, and dynamic routing
2. Create `app/(chat)/layout.tsx` with sidebar integration
...
1. **Analyze `archive/oldapp/app/api/`** directory to understand existing API route patterns, error handling, and response formats
2. Create `app/api/auth/callback/route.ts` OAuth token exchange (GET)
...
1. **Analyze `archive/oldapp/app/api/`** for existing chat API route patterns, streaming implementation, and pagination handling
2. Create `app/api/chat/route.ts` main chat streaming (POST)
...
1. **Analyze `archive/oldapp/app/api/`** for existing artifact, file upload, history, suggestions, and vote API patterns
2. Create `app/api/artifact/route.ts` artifact CRUD
...
*Tasks: 8 | Est. Duration: 10-14 hours*
...
1. **Analyze `archive/oldapp/lib/middleware/`** for existing middleware patterns, route matching, and rate limit integration
2. Create `middleware.ts` with Upstash rate limiting
...
1. **Analyze `archive/oldapp/instrumentation.ts`** and `archive/oldapp/instrumentation-client.ts`** for existing instrumentation patterns and initialization logic
2. Create `instrumentation.ts` with `register` function export
...
1. **Analyze `archive/oldapp/tests/`** directory for existing test patterns, fixtures, and helpers
2. **Analyze `archive/oldapp/playwright.config.ts`** for E2E test configuration
3. Create `src/test/setup.ts` Vitest global setup
...
**AI Mocking Strategy:**
- [ ] Create `__mocks__/ai.ts` with mock streamText returning deterministic responses
- [ ] Mock tool calls with predictable JSON outputs
- [ ] Implement streaming delay simulation (50ms chunks)
- [ ] Handle partial response scenarios for edge case testing
- [ ] Create fixture responses for each artifact type (text, code, image, sheet)

**Coverage Requirements:**
- [ ] Configure Jest coverage threshold at 70% (branches, functions, lines)
- [ ] Add coverage check to CI pipeline (fail build if under threshold)
- [ ] Exclude `components/ai-elements/` from coverage (read-only primitives)
...
### Task 6.3c – Visual Regression Testing - Agent_Testing
**Objective:** Set up visual regression testing for UI components.
**Output:** Playwright visual comparison tests, baseline screenshots.
**Guidance:** **Depends on: Task 6.5 Output**. Use Playwright's screenshot comparison.

1. Configure Playwright for visual regression (`toMatchSnapshot`)
2. Create baseline screenshots for critical components:
   - Chat interface (empty, with messages, streaming)
   - Artifact panel (text, code, image, sheet editors)
   - Sidebar (collapsed, expanded)
   - Auth forms (login, register)
3. Set threshold for acceptable visual diff (0.1%)
4. Add visual regression to CI pipeline
...
|----------|-------|
| **Total Phases** | 7 |
| **Total Tasks** | 75 | <!-- Updated: +1 (Task 0.2) +2 (Task 4.5b, 4.7b) +1 (Task 6.3c) +1 (Task 1.2b) = 70+5 -->
| **Single-step Tasks** | 28 |
| **Multi-step Tasks** | 47 |
| **Cross-Agent Dependencies** | 25 |

| Agent | Task Count |
|-------|------------|
| Agent_Infrastructure | 15 | <!-- +1 for Task 0.2, +1 for Task 1.2b -->
| Agent_Data | 16 |
| Agent_Features | 17 |
| Agent_Components | 12 | <!-- +2 for Task 4.5b, 4.7b -->
| Agent_AppRouter | 6 |
| Agent_Testing | 9 | <!-- +1 for Task 6.3c -->
