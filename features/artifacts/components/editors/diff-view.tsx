...
export function DiffView({ oldContent, newContent }: DiffViewProps) {
  ...
  return <DiffViewInner modules={modules} newContent={newContent} oldContent={oldContent} />;
}
...
function DiffViewInner({ oldContent, newContent, modules }: DiffViewInnerProps) {
  const {
    Editor,
    Extension,
    Mark,
