...
const getMetadataCacheKey = (documentId: string) =>
  documentId && documentId !== 'init' ? `artifact-metadata-${documentId}` : null;
 ...
 * ```
 */
export function useArtifactSelector<TSelected>(selector: Selector<TSelected>): TSelected {
  const [mounted, setMounted] = useState(false);

  const { data: localArtifact } = useSWR<UIArtifact>(ARTIFACT_CACHE_KEY, null, {
...
export function useArtifact(): UseArtifactReturn {
  ...
  const { data: localArtifactMetadata, mutate: setLocalArtifactMetadata } =
    useSWR<ArtifactMetadata>(getMetadataCacheKey(artifact.documentId), null, {
      fallbackData: null,
  ...
  const setMetadata = useCallback(
    (updaterFn: SetStateAction<ArtifactMetadata>) => {
      if (typeof updaterFn === 'function') {
        setLocalArtifactMetadata((currentMetadata: ArtifactMetadata) => {
          return (updaterFn as (prev: ArtifactMetadata) => ArtifactMetadata)(
