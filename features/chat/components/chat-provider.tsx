 ...
 */
const ChatHelpersContext = createContext<UseChatHelpers<ChatMessage> | null>(null);
  ...
  const chatHelpers = useChat<ChatMessage>({
    ...
    onData: (data) => {
      ...
      if (streamData.type === 'data-chat-title') {
        ...
        console.debug('[ChatProvider] Title received:', streamData.data);
      }
      if (streamData.type === 'data-usage') {
        // Handle usage tracking
        const usage = streamData.data as { inputTokens?: number; outputTokens?: number; totalTokens?: number } | undefined;
