...
function normalizeMessagePart(part: unknown): MessagePartType | null {
  ...
  switch (p.type) {
    ...
    case 'reasoning':
      return {
        type: 'reasoning',
        reasoning: typeof p.text === 'string' ? p.text : (typeof p.reasoning === 'string' ? p.reasoning : ''),
      };
...
export function MessageContent({
  ...
  const hasParts = 'parts' in message && Array.isArray(message.parts) && message.parts.length > 0;
      ...
      <div className={cn('flex flex-col gap-2', className)}>
        ...
        {textParts.length > 0 && (
          <div
            className={cn(
              'break-words',
              isUser && 'w-fit rounded-2xl bg-[#006cff] px-3 py-2 text-white',
            ...
            <div className="space-y-2">
              {textParts.map((part, index) => (
                <MessagePart
                  ...
                  part={part}
                  isStreaming={isStreaming && index === textParts.length - 1}
                />
              ))}
            </div>
          </div>
        ...
        {otherParts.map((part, index) => (
          <MessagePart
            ...
            part={part}
            isStreaming={isStreaming && index === otherParts.length - 1}
          />
        ))}
      </div>
    );
