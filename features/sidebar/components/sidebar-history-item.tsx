...
import { MoreHorizontal, Share, Lock, Globe, Trash2, Check } from 'lucide-react';
...
export const SidebarHistoryItem = memo(function SidebarHistoryItem({
  ...
  const [visibility, setVisibility] = useState<VisibilityType>(chat.visibility || 'private');
  ...
  const handleVisibilityChange = async (newVisibility: VisibilityType) => {
    ...
    try {
      const response = await fetch('/api/chat/visibility', {
        ...
        body: JSON.stringify({ chatId: chat.id, visibility: newVisibility }),
      });
