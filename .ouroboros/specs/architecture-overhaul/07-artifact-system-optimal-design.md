...
ArtifactShellProps) {
  ...
  const { document, saveContent, isContentDirty } = useArtifactPersistence(
    artifact.documentId
  );
...
registerArtifact("code", () =>
  import("@/artifacts/code/client").then((m) => m.codeArtifact)
);
registerArtifact("text", () =>
  import("@/artifacts/text/client").then((m) => m.textArtifact)
);
registerArtifact("image", () =>
  import("@/artifacts/image/client").then((m) => m.imageArtifact)
);
registerArtifact("sheet", () =>
  import("@/artifacts/sheet/client").then((m) => m.sheetArtifact)
);
```
...
export function useArtifactPersistence(documentId: string) {
  ...
  const saveContent = useCallback(
    async (content: string, debounce: boolean) => {
...
export function useArtifactVersioning(documentId: string) {
  ...
  const isCurrentVersion =
    !documents?.length || currentVersionIndex === documents.length - 1;
...
const editorLoaders = {
  ...
  image: lazy(() =>
    import("@/components/image-editor").then((m) => ({
...
export function ArtifactContent({
  ...
  const EditorComponent = useMemo(
    () => editorLoaders[kind] ?? editorLoaders.text,
    [kind]
  );
