...
> [!CAUTION] > **EVERY task/phase/continuance MUST begin with this checklist. NO EXCEPTIONS.**

| #   | Action                       | File/Location                                                                |
| --- | ---------------------------- | ---------------------------------------------------------------------------- |
| 1   | [ ] Read core instructions   | `prompt-genome/ULTIMATE-FINAL-PROMPT.md`                                     |
| 2   | [ ] Read architecture plan   | `.ouroboros/specs/architecture-overhaul/FINAL-ARCHITECTURE-OVERHAUL-PLAN.md` |
| 3   | [ ] Read relevant spec       | `.ouroboros/specs/architecture-overhaul/XX-module-optimal-design.md`         |
| 4   | [ ] Read current progress    | `.context/state.json`                                                        |
| 5   | [ ] Read active task/subtask | Path from `state.json → active.task/subtask`                                 |

**IF YOU SKIP THIS CHECKLIST → PROTOCOL VIOLATION → STOP AND RESTART**

---

## 🎨 UI VISUAL PARITY (ABSOLUTE RULE)

> [!WARNING] > **The NewApp MUST be visually IDENTICAL to OldApp. This is NON-NEGOTIABLE.** > **How the code is written UNDER THE HOOD can differ, but the USER EXPERIENCE MUST MATCH 100%.** > **Optimization under the hood is ALLOWED, but visual/UX changes are FORBIDDEN.**
...
> [!CAUTION] > **Before ANY implementation begins, create the master task list.**
...
| -------- | -------------------- | ------------------ | ------ | -------------- |
| AUTH-001 | JWT token generation | None               | M      | ✅ Complete    |
| AUTH-002 | Session management   | AUTH-001           | L      | 🔄 In Progress |
| AUTH-003 | Auth guards          | AUTH-001, AUTH-002 | M      | ⏳ Not Started |
| AUTH-004 | Login form           | AUTH-003           | S      | ⏳ Not Started |
```
...
> [!IMPORTANT] > **This is the EXACT format for `.context/state.json`. No deviations allowed.**
...
> [!CAUTION] > **Before implementing module X, you MUST complete this sequence.**

1. [ ] **Read** `FINAL-ARCHITECTURE-OVERHAUL-PLAN.md` — Section for module X
