...
| 6 | [auth-system.md](auth-system.md) | **Auth System** — Dual auth architecture (Supabase + Guest JWT), session resolution, token exchange flow via `exchangeTokenAction()` Server Action, guest auto-creation in proxy.ts, token rotation, guest limitations table, rate limiting, auth guards, security considerations (IDOR, JWT, cookies). SessionProvider wraps app. |
