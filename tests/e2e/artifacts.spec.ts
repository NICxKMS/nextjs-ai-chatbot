...
test.describe("Artifacts", () => {
    ...
    test.describe("Code Artifact", () => {
        test("should trigger code artifact creation via message", async ({
            ...
            const isVisible = await artifact
        ...
        test("should have close button in artifact panel", async ({ page }) => {
            ...
            const artifact = page.getByTestId("artifact");
            if (
                await artifact.isVisible({ timeout: 5000 }).catch(() => false)
            ) {
        ...
        test("should close artifact when close button clicked", async ({
            page,
        }) => {
            const input = page.getByPlaceholder(/send a message/i);
            const sendButton = page.getByTestId("send-button");
    ...
    test.describe("Text Artifact", () => {
        test("should trigger text artifact via document request", async ({
            page,
        }) => {
            const input = page.getByPlaceholder(/send a message/i);
            const sendButton = page.getByTestId("send-button");
    ...
    test.describe("Version History", () => {
        test("should show version footer when artifact has versions", async ({
            page,
        }) => {
            const input = page.getByPlaceholder(/send a message/i);
            const sendButton = page.getByTestId("send-button");
            ...
            if (
                ...
                const hasVersionFooter = await versionFooter
        ...
        test("should navigate between versions when multiple exist", async ({
            page,
        }) => {
