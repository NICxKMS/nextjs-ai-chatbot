...
test.describe("Chat", () => {
  ...
  test.describe("New Chat Creation", () => {
    ...
    test("should display suggested actions for new chat", async ({ page }) => {
      // Look for suggestion buttons or suggested actions container
      const suggestedActions = page.getByTestId("suggested-actions");
      if (await suggestedActions.isVisible({ timeout: 5000 }).catch(() => false)) {
  ...
  test.describe("Message Sending", () => {
    ...
    test("should redirect to /chat/:id after sending message", async ({ page }) => {
      const input = page.getByPlaceholder(/send a message/i);
      const sendButton = page.getByTestId("send-button");
    ...
    test("should display assistant message after sending", async ({ page }) => {
      const input = page.getByPlaceholder(/send a message/i);
      const sendButton = page.getByTestId("send-button");
  ...
  test.describe("Chat History", () => {
    test("should persist chat in history after sending message", async ({ page }) => {
      const input = page.getByPlaceholder(/send a message/i);
      const sendButton = page.getByTestId("send-button");
  ...
  test.describe("Model Selection", () => {
    ...
    test("should open model selector dropdown on click", async ({ page }) => {
      const modelSelector = page.getByTestId("model-selector");
      await modelSelector.click();

      // Dropdown items should be visible
      const modelItems = page.locator("[data-testid^='model-selector-item-']");
