...
test.describe('Documents', () => {
  ...
  test.describe('Document Preview', () => {
    test('should display document preview in message when created', async ({ page }) => {
      ...
      const hasPreview = await documentPreview.isVisible({ timeout: 5000 }).catch(() => false);
      expect(typeof hasPreview).toBe('boolean');
    });

    test('should open artifact when clicking document preview', async ({ page }) => {
      const input = page.getByPlaceholder(/send a message/i);
      const sendButton = page.getByTestId('send-button');
  ...
  test.describe('Document Tool Call', () => {
    test('should display document tool call indicator', async ({ page }) => {
      const input = page.getByPlaceholder(/send a message/i);
      const sendButton = page.getByTestId('send-button');
      ...
      const hasToolCall = await toolCall.isVisible({ timeout: 5000 }).catch(() => false);
      expect(typeof hasToolCall).toBe('boolean');
    });

    test('should show loading state during document creation', async ({ page }) => {
      const input = page.getByPlaceholder(/send a message/i);
      const sendButton = page.getByTestId('send-button');
      ...
      const hasLoader = await loader.isVisible({ timeout: 3000 }).catch(() => false);
