...
describe('AppError', () => {
  ...
  it('should accept custom status code', () => {
    const error = new AppError({ code: 'auth:unauthorized', statusCode: 403 });
    expect(error.statusCode).toBe(403);
  });

  it('should serialize to Response', () => {
    const error = new AppError({ code: 'validation:invalid_input', message: 'Bad data' });
  ...
  it('should serialize to ActionResult', () => {
    const error = new AppError({ code: 'auth:unauthorized', message: 'Not allowed' });
