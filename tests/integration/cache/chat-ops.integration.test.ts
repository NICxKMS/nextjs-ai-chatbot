...
describeIf(
    isServiceAvailable("cache"),
    "Chat Cache Operations Integration",
    () => {
        ...
        describe("createChatInCache", () => {
            ...
            it("creates a guest chat with TTL", async () => {
                ...
                const retrieved = await getChatFromCache(
                    chatData.id,
                    TEST_USER_ID
                );
        ...
        describe("deleteChatFromCache", () => {
            it("deletes an existing chat", async () => {
                ...
                // Verify it exists
                const beforeDelete = await getChatFromCache(
                    chatData.id,
                    TEST_USER_ID
                );
                ...
                // Delete it
                const deleteResult = await deleteChatFromCache(
                    chatData.id,
                    TEST_USER_ID
                );
                ...
                // Verify it's gone
                const afterDelete = await getChatFromCache(
                    chatData.id,
                    TEST_USER_ID
                );
        ...
        describe("Full chat lifecycle", () => {
            it("create -> read -> update -> delete workflow", async () => {
                ...
                const afterUpdate = await getChatFromCache(
                    chatData.id,
                    TEST_USER_ID
                );
                ...
                // Delete
                const deleted = await deleteChatFromCache(
                    chatData.id,
                    TEST_USER_ID
                );
