...
| `echo "..." > Dockerfile` | Use VS Code `edit` tool |
| `Set-Content -Path "..." -Value "..."` | Use VS Code `edit` tool |
| `New-Item -Path "..." -Value "..."` | Use VS Code `edit` tool |
| `cat > file.yml << EOF` | Use VS Code `edit` tool |
| Any terminal command that creates/modifies config files | Use VS Code `edit` tool |

**RULE**: Use the VS Code `edit` tool for ALL file creation and modification. Terminal is ONLY for running Git commands, CI/CD commands, and Docker commands.

**EXCEPTION**: Git operations (`git add`, `git commit`, `git push`) are allowed as they don't create files directly.
