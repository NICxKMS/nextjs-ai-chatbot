...
export const patchTextNodes = (schema, oldNode, newNode) => {
  ...
  // Map diffs to nodes
  const res = diffs.flatMap(([type, text]) => {
    if (!text) {
      return [];
    }
...
const wordsToChars = (text1, text2) => {
  const lineArray = []; // Actually stores words
  const lineHash = {};
...
const charsToWords = (diffs, lineArray) => {
  return diffs.map(([type, text]) => {
    const words = [];
    for (let i = 0; i < text.length; i++) {
      words.push(lineArray[text.charCodeAt(i)]);
    }
