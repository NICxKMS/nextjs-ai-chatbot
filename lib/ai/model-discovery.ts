...
const fetchJson = async <T>(
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch ${input.toString()}: ${response.status}`);
  }
    ...
    async () => {
      ...
      const models = data.data.map((model) =>
        mapModel("openai", {
          id: `openai:${model.id}`,
          modelId: model.id,
          release: new Date(model.created * 1000).toISOString().slice(0, 10),
          tags: [model.owned_by],
        })
      );
    ...
    async () => {
      ...
      const models = data.models.map((model) => {
        ...
        return mapModel("google", {
          ...
          capabilities: model.supportedGenerationMethods?.includes("reasoning")
            ? ["chat", "reasoning"]
            : ["chat"],
          modalities:
    ...
    async () => {
      ...
      const models = data.result.map((model) =>
        mapModel("cloudflare-workers", {
          ...
          capabilities: model.meta.capabilities?.map((cap) => cap as any) ?? [
            "chat",
          ],
        })
      );
