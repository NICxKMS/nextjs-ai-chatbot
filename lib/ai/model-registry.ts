...
if (OPENROUTER_API_KEY) {
  registerProvider(
    "openrouter",
    createOpenRouter({ apiKey: OPENROUTER_API_KEY }) as unknown as ProviderV2
  );
}
...
export const refreshModelCatalog = async (options?: { force?: boolean }) => {
  ...
  try {
    ...
    modelCatalog =
      merged.length > 0 ? merged : filterByAvailableProviders(curatedModels);
  } catch (error) {
...
export const getReasoningModel = () => {
  ...
  return (
    modelCatalog.find((model) => model.capabilities.includes("reasoning")) ??
    getDefaultChatModel()
  );
};
