...
import {
    ...
    recordCacheHit as recordRequestCacheHit,
    recordCacheMiss as recordRequestCacheMiss,
 ...
 * Integrates with both global metrics and per-request tracking
 */
export async function withCacheMetrics<T>(
    ...
    try {
        ...
        if (result !== null && result !== undefined) {
            ...
            recordRequestCacheHit(latency);
        } else {
            CacheMetrics.recordMiss(operation, latency);
            recordRequestCacheMiss(latency);
        }
...
/**
 * Helper to wrap cache write operations with metrics tracking
 * Write operations don't have hit/miss semantics
 */
export async function withCacheWriteMetrics<T>(
    operation: string,
    fn: () => Promise<T>
): Promise<T> {
    const start = performance.now();
    ...
    try {
        ...
        // Record as operation count (no hit/miss for writes)
        recordCacheOperation(latency);
        ...
        if (span) {
            ...
            span.setAttribute("cache.latency_ms", latency);
        }
