...
import { getDocument, getAllDocuments, saveDocument, deleteDocumentsAfterTimestamp } from "../documents";
...
export async function deleteDocumentCached(
  ...
  const deleted = await deleteDocumentsAfterTimestamp(documentId, new Date(0), ctx);
  if (deleted.length > 0) {
