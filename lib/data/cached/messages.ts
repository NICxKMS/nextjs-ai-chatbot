...
import { getDb, schema, type Message } from "@/lib/db";
...
export async function getMessagesCached(
  ...
  if (messages.length > 0) {
    appendMessagesToCache(chatId, ctx.userId, messages.map(messageToCached), false).catch(() => {});
  }
...
export async function deleteMessagesAfterTimestampCached(
  ...
  await db
    ...
    .execute();
  const count = await deleteMessagesAfterTimestamp(chatId, ctx.userId, afterTimestamp);
  return count;
}
