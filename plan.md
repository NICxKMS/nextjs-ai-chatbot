# 🧬 ULTIMATE FINAL PROMPT

> **Generation 23** | **ERA 3 ULTIMATE** | **Production Ready** | **Works with .github/agents/**
...
| #   | Check                 | Action                                            |
| --- | --------------------- | ------------------------------------------------- |
| 1   | [ ] Core Instructions | Read `prompt-genome/ULTIMATE-FINAL-PROMPT.md`     |
| 2   | [ ] Architecture Plan | Read `FINAL-ARCHITECTURE-OVERHAUL-PLAN.md`        |
| 3   | [ ] Module Spec       | Read relevant `XX-module-optimal-design.md`       |
| 4   | [ ] Current Progress  | Read `.context/state.json`                        |
| 5   | [ ] Active Task       | Read file from `state.json → active.task/subtask` |

**FAILURE TO COMPLETE CHECKLIST = PROTOCOL VIOLATION = STOP AND RESTART**

---

## 🎨 UI VISUAL PARITY (ABSOLUTE RULE)
...
| Changes (Under the Hood)  | Stays Same (User-Facing) |
| ------------------------- | ------------------------ |
| Component architecture    | Visual appearance        |
| State management patterns | Color scheme             |
| Server/Client boundaries  | Typography               |
| File organization         | Spacing/Layout           |
| Caching strategy          | Animations/Transitions   |
| Error handling            | User interactions        |

**Verification Method:**

- Use React DevTools to compare component output
- Screenshot comparison between OldApp and NewApp
- A/B test with users — they should notice NO difference

---

## 🔄 VERCEL AI SDK ELEMENTS (COPY & ADAPT)
...
1. **COPY** the OldApp implementation exactly
2. **RELOCATE** to new directory structure
3. **UPDATE** to adapt to new architecture
4. **PRESERVE** functionality**TEST** streaming behavior matches OldApp exactly

## 📝 ON PROJECT START (MANDATORY TASK LIST)

> ⚠️ **Before ANY implementation begins, create the master task list.**

**BEFORE writing a single line of code:**

1. **Create** `.ouroboros/specs/[feature]/00-MASTER-TASK-LIST.md`
2. **Break down** ALL tasks with required fields
3. **Order** tasks by dependency graph
4. **Update** this list BEFORE and AFTER each task

### Task List Required Format

```markdown
# [Feature] Master Task List
...
| ------- | ---------------- | ---------------- | ------ | -------------- | ------------ |
| XXX-001 | Task description | None             | S      | ✅ Complete    |              |
| XXX-002 | Task description | XXX-001          | M      | 🔄 In Progress | Blocker: X   |
| XXX-003 | Task description | XXX-001, XXX-002 | L      | ⏳ Not Started |              |
| XXX-004 | Task description | XXX-003          | S      | 🚫 Blocked     | Waiting on Y |
...
## 📊 CONTEXT/PROGRESS TRACKER FORMAT (EXACT)

> **This is the EXACT format for `.context/state.json`. No deviations allowed.**
...
| **AFTER completing task** | Update all `progress` counts, `safe_to_interrupt: true` |
| **ALWAYS**                | Update `last_action` and `next_action`                  |
| **On session end**        | `status: "paused"`, `safe_to_interrupt: true`           |
| **On error/block**        | `status: "blocked"`, document blocker in task file      |

---

## 📚 SPEC-READING ENFORCEMENT (NON-NEGOTIABLE)

> ⚠️ **Before implementing module X, you MUST complete this sequence.**

### Required Reading Sequence

1. [ ] **Read** `FINAL-ARCHITECTURE-OVERHAUL-PLAN.md` — Find section for module X
2. [ ] **Read** `XX-module-name-optimal-design.md` — **Read COMPLETELY, not skimming**
3. [ ] **Note** all:
   - Required interfaces
   - Specified patterns
 ...
 *
 * Implements: ACCESS_TOKEN with 30min expiry, REFRESH_TOKEN with 7day expiry
 */
export function generateTokenPair(userId: string): TokenPair {
  // Implementation following spec §3.2.1
}
```

**IMPLEMENTING WITHOUT READING SPEC = PROTOCOL VIOLATION**
